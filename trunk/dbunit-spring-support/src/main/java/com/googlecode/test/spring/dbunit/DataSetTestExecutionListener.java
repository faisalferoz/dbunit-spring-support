...
import com.googlecode.test.dbunit.DBUnitUtils;
    ...
    extends AbstractTestExecutionListener
{
    ...
    private final Map<Method, DatasetConfiguration> configurationCache =
        Collections.synchronizedMap( new IdentityHashMap<Method, DatasetConfiguration>() );
        ...
        throws Exception
    {
        ...
        // Fetch a connection from the data source, using an existing one if we're already participating in a
        // transaction.
        ...
        throws Exception
    {
        ...
        if ( !configuration.isConnectionTransactional() )
        {
            ...
            {
                // do nothing: this connection is associated with an active transaction and we assume the framework will
    ...
    DataSource lookupDataSource( TestContext testContext, String datasourceName )
    {
        ...
        if ( dsNames.length > 1 )
        {
            // if there are more that one datasources use the one specified in the DataSet annotation.
     ...
     * 
     * @param configuration the spring-style resource locations of the dataset to be loaded
        ...
        throws Exception
    {
        ...
        tester.getConnection().getConfig().setProperty( DatabaseConfig.PROPERTY_DATATYPE_FACTORY,
        ...
        final CompositeDataSet dataSet = new CompositeDataSet( dataSets.toArray( new IDataSet[0] ) );
